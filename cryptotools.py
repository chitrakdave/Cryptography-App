...
# a. AES Encryption function based on CBC mode
...
# b. AES Decryption function based on CTR mode
...
# ---- helper to avoid ZeroDivisionError on ultra-fast ops ----
def safe_speed(bytes_count, duration):
    # duration can be 0 for very fast operations; return 0.0 to avoid division by zero
    return (bytes_count / duration) if duration > 0 else 0.0
# -------------------------------------------------------------
...
for bitLength in bitSizeList:
    ...
    for fileKey, file in fileData.items():
        ...
        speed = safe_speed(fileStorage[fileKey], duration)
        totalSpeed_encrypt_CBC += speed
        print(f'Speed per byte to encrypt for {fileKey} using {bitLength}-bit AES CBC mode {speed} bytes/ns')
        ...
        speed = safe_speed(fileStorage[fileKey], duration)
        totalSpeed_decrypt_CBC += speed
        print(f'Speed per byte to decrypt for {fileKey} using {bitLength}-bit AES CBC mode {speed} bytes/ns')
        ...
        speed = safe_speed(fileStorage[fileKey], duration)
        totalSpeed_encrypt_CTR += speed
        print(f'Speed per byte to encrypt for {fileKey} using {bitLength}-bit AES CTR mode {speed} bytes/ns')
        ...
        speed = safe_speed(fileStorage[fileKey], duration)
        totalSpeed_decrypt_CTR += speed
        print(f'Speed per byte to decrypt for {fileKey} using {bitLength}-bit AES CTR mode {speed} bytes/ns \n')
...
for rsaKey in RSAkey_length:
    ...
    for fileKey,file in fileData.items():
        ...
        msgLen = rsaKey//8 - 42  # OAEP (SHA-1) overhead assumption (original code intent)
        out = []
        startTime = time()
        ...
        duration = round((time()-startTime)*10**6,2)  # microseconds
        totalTime_encrypt_RSA += duration
        
        print(f'Time Taken to encrypt {fileKey} using RSA {rsaKey}-bit {duration} micro seconds')

        speed = safe_speed(fileStorage[fileKey], duration)
        totalSpeed_encrypt_RSA += speed
        print(f'Speed per byte to encrypt for {fileKey} using RSA {rsaKey}-bit {speed} bytes/micro seconds')
        ...
        speed = safe_speed(fileStorage[fileKey], duration)
        totalSpeed_decrypt_RSA += speed
        print(f'Speed per byte to decrypt for {fileKey} using RSA {rsaKey}-bit {speed} bytes/micro seconds\n')
...
for hashKey,hashType in hashTypes.items():
    ...
    for fileKey,file in fileData.items():
        ...
        speed = safe_speed(fileStorage[fileKey], duration)
        totalSpeed_hash += speed
        print(f'Speed per byte to hash for {fileKey} using {hashKey} technique {speed} bytes/micro seconds \n')
...
for DSAKey in DSAkey_length:
    ...
    for fileKey,file in fileData.items():
        ...
        duration = round((time()-startTime)*10**9,2)  # ns
        totalTime_Sign += duration
        print(f"Time taken to generate signature for {fileKey} by DSA-{DSAKey} bit {duration} ns")

        speed = safe_speed(fileStorage[fileKey], duration)
        totalSpeed_Sign += speed
        print(f'Speed per byte to sign for {fileKey} using DSA-{DSAKey} bit  {speed} bytes/ns')
        ...
        speed = safe_speed(fileStorage[fileKey], duration)
        totalSpeed_verify += speed
        print(f'Speed per byte to verify signature for {fileKey} using DSA-{DSAKey} bit  {speed} bytes/ns \n')
